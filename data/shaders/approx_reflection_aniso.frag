...
vec4 mn = vec4(-0.95592842, -1.80896948, -1.42290871,  0.79290551);


float sigmoid2d_x(float x,float y){
    x = clamp(x,0.0,1.0);
    y = clamp(y,0.0,1.0);
    return (mn.w*x/(1.0 + exp(-mn.x*y+mn.z))) * (1.0/(1.0 + exp(-mn.y*x+mn.z)));
}

float sigmoid2d_y(float x,float y){
    x = clamp(x,0.0,1.0);
    y = clamp(y,0.0,1.0);
    return (mn.w*y/(1.0 + exp(-mn.x*x+mn.z))) * (1.0/(1.0 + exp(-mn.y*y+mn.z)));
}

vec2 roughnessToVariance(float alpha_x,float alpha_y){
    // return vec2(sigmoid2d_x(alpha_x,alpha_y),sigmoid2d_y(alpha_x,alpha_y));
    return texture2D(u_roughnessToVariance, vec2(alpha_x,alpha_y)).xy;
}
...
void main(void)
{
    ...
    if(sinTo <= 1.0) {
        ...
        vec2 alpha_xy_approx = varianceToRoughness(roughnessToVariance(u_alpha_x_1,u_alpha_y_1));

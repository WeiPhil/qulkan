...
namespace Qulkan::Vulkan {
    ...
    SimpleView::SimpleView(VkInstance instance, VKHelper::Device aDevice, VKHelper::Queue graphicsQueue, VkExtent2D anExtent, VkFormat aFormat,
                           const char *viewName)
        : RenderView(viewName, anExtent.width, anExtent.height), device(aDevice), format(aFormat), extent(anExtent), commandPool(aDevice, graphicsQueue, 1),
    ...
    ImTextureID SimpleView::render() {
        VK_CHECK_RET_NULL(renderer.drawFrame(std::vector<VkCommandBuffer>{commandBuffer}));

...
class Logger {
  ...
  public:
    ...
    template <typename... Args> static void Error(const char *fmt, Args... args) { Instance().Log(LogLevel::ERROR, fmt, args...); }
    ...
    void Window(bool *p_open = NULL) {
        ...
        if (Filter.IsActive()) {
            ...
            // When using the filter (in the block of code above) we don't have random access into the data to display anymore, which is why we don't use the
            // clipper. Storing or skimming through the search result would make it possible (and would be recommended if you want to search through tens of
